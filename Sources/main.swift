...
final class AppState: ObservableObject {
    ...
    func loadSettings() {
        ...
        remoteEnabled = cfgBool("remoteEnabled", false)
        remoteTopic = UserDefaults.standard.string(forKey: "remoteTopic") ?? ""
        remoteToken = UserDefaults.standard.string(forKey: "remoteToken") ?? ""
    }
    ...
    func saveSettings() {
        ...
        // topic/token are generated by RemoteControl.ensureCredentials and are
        // only read here, so they're not overwritten on save.
        saveRules(rules)
    }
...
// MARK: - RemoteHost (phone app / widget)

extension AppDelegate: RemoteHost {
    func remoteSetMode(_ m: Mode) {
        log("remote -> setMode \(m.rawValue)")
        state.addEvent("Phone: switched to \(m.title)")
        setMode(m)
    }

    func remoteSleepNow() {
        log("remote -> sleep now")
        state.addEvent("Phone: Sleep Now")
        sleepNow()
    }

    // Compact status snapshot the phone app + widget render.
    func remoteStatsSnapshot() -> [String: Any] {
        return [
            "mode": mode.rawValue,
            "awake": state.awake,
            "veto": state.vetoReason ?? "",
            "battery": state.battery.percent,
            "charging": state.battery.charging,
            "hasBattery": state.battery.present,
            "temp": state.tempC.map { Int($0.rounded()) } ?? -1,
            "cpu": state.cpuUsage ?? -1,
            "thermal": state.thermal.label,
            "lidClosed": state.lidClosed,
            "reasons": state.reasons,
            "holds": state.holds.count,
